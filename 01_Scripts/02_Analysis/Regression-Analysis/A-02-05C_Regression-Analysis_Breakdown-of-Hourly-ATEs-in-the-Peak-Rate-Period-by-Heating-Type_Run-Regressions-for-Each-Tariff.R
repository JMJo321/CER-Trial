...
list_number.of.households_by.heating.type <- lapply(list_cases, get_number.of.households)


# # 2. Run regression(s)
list_reg.results_ate <- lapply(
  list_cases,
  get_reg.result,
...
add.lines <- list(
  c("Electrical Heating for Space", c("True", "False", "False")),
  c("Electrical Heating for Water", c("True", "True", "False")),
  c("FEs: Household by Half-Hourly Time Window", rep("Yes", times = 6)),
  c("FEs: Day of Week by Half-Hourly Time Window", rep("Yes", times = 6)),
  c("FEs: Month of Year", rep("Yes", times = 6)),
  c(
...
stargazer(
  ...
  out = paste(
    ...
    "ATEs_Hourly-in-the-Peak-Rate-Period_By-Heating-Type.tex",
    sep = "/"
  ),
  out.header = out.header,
  column.labels = column.labels,
...
reg.result_by.heating.type <- lapply(
  list_cases,
  get_reg.result,
...
add.lines <- list(
  c(
    ...
    rep(c("True", "False", "False"), times = 1)
  ),
  c("Electrical Heating for Water", rep(c("True", "True", "False"), times = 1)),
  c("FEs: Household by Half-Hourly Time Window", rep("Yes", times = 3)),
  ...
  c(
    "Number of Households",
    as.vector(list_number.of.households, mode = "numeric") %>%
      format(., big.mark = ",") %>%
      rep(., each = 1)
  )
)
...
for (tariff in LETTERS[1:4]) {
  ...
  obj.name_reg.result <-
    paste0("reg.result_by.heating.type.and.tariff_", tolower(tariff))
...
add.lines <- list(
  c("Tariff", rep(LETTERS[1:4], each = 3)),
  c(
    "Electrical Heating for Space",
    rep(c("True", "False", "False"), times = 4)
  ),
  c("Electrical Heating for Water", rep(c("True", "True", "False"), times = 4)),
  ...
  c(
    "Number of Households",
    list_number.of.households_by.tariff.and.heating.type
  )
)
label <- paste0(
  "Table:Breakdown-of-Average-Treatment-Effects-in-the-Peak-Rate-Period_",
...
stargazer(
  c(
    reg.result_by.heating.type.and.tariff_a,
    reg.result_by.heating.type.and.tariff_b,
    reg.result_by.heating.type.and.tariff_c,
    reg.result_by.heating.type.and.tariff_d
  ),
  type = "text",
  title = title_,
  out = NULL,
...
stargazer(
  ...
  out = paste(
    ...
    paste0(
      ...
      "By-Tariff-and-Heating-Type.tex"
    ),
    sep = "/"
  ),
  out.header = out.header,
...
for (knot in knots) {
  ...
  obj.name <- paste0("list_reg.results_by.heating.type_knot", knot)
  ...
  # ### Object(s) that will be used to make stargazer object(s)
  title_ <- paste0(
    "Breakdown of Average Treatment Effects in the Peak Rate Period, ",
  ...
  # ### Export the regression table(s) in .tex format
  stargazer(
    get(obj.name),
    type = "latex",
    title = title_,
    out = paste(
      DIR_TO.SAVE_LATEX_BREAKDOWN,
      paste0(
        "Breakdown-of-ATEs_Hourly-in-the-Peak-Rate-Period_By-Heating-Type_",
        "Knot-", knot, ".tex"
      ),
      sep = "/"
    ),
...
list_plots_treatment.effects_knot10 <- mapply(
  ...
  list_reg.results_by.heating.type_knot10,
  list_cases,
  knot = 10,
  SIMPLIFY = FALSE
)
list_plots_treatment.effects_knot15 <- mapply(
  get_ggplot.obj_treatment.effects,
  list_reg.results_by.heating.type_knot15,
  list_cases,
  knot = 15,
  SIMPLIFY = FALSE
)
...
export_figure.in.png(
  ...
  filename_str = paste(
    ...
    paste0(
      ...
      "Treatment-Effect-by-Heating-Type_By-using-Spline-Regressions.png"
    ),
    sep = "/"
  ),
...
export_figure.in.png(
  ...
  filename_str = paste(
    ...
    paste0(
      "Figure_Breakdown-of-Hourly-ATEs-in-the-Peak-Rate-Period_",
      "Load-Profile-by-Heating-Type_By-using-Spline-Regressions.png"
    ),
    sep = "/"
  ),
  width_numeric = 45, height_numeric = 25
...
# # ------- Run regression(s), by tariff and heating type -------
# # 1. Run regression(s), make stargazer object(s), and export the regression
# #    table
for (knot in knots) {
  modified.formula <- get_change.terms.in.formula(
    model_breakdown.of.ate_hourly.in.peak_splines_iw.dw.mw,
  ...
  for (tariff in LETTERS[1:4]) {
    obj.name_reg.result <- paste0("tmp_reg.result_", tolower(tariff))
    obj.name_number.of.households <-
      paste0("tmp_number.of.households_", tolower(tariff))
    assign(
      obj.name_reg.result,
      lapply(
        list_cases,
  ...
  list_number.of.households_ <- c(
    as.vector(tmp_number.of.households_a, mode = "numeric") %>%
      format(., big.mark = ","),
    as.vector(tmp_number.of.households_b, mode = "numeric") %>%
      format(., big.mark = ","),
    as.vector(tmp_number.of.households_c, mode = "numeric") %>%
      format(., big.mark = ","),
    as.vector(tmp_number.of.households_d, mode = "numeric") %>%
      format(., big.mark = ",")
  )
  title_ <- paste0(
    "Breakdown of Average Treatment Effects in the Peak Rate Period, ",
    "By Tariff and Heating Type, For Knot = ", knot
  )
  covariate.labels_text <- c(
    "HDDs",
    "(HDDs - Knot) x 1[HDDs > Knot]",
  ...
  add.lines <- list(
    ...
    c(
      ...
      rep(c("True", "True", "False"), times = 4)
    ),
    c("FEs: Household by Half-Hourly Time Window", rep("Yes", times = 12)),
    c("FEs: Day of Week by Half-Hourly Time Window", rep("Yes", times = 12)),
    c("FEs: Month of Year by Half-Hourly Time Window", rep("Yes", times = 12)),
    c("Number of Households", list_number.of.households_)
  )
  label <- paste0(
    "Table:Breakdown-of-Average-Treatment-Effects-in-the-Peak-Rate-Period_",
    "By-Tariff-and-Heating-Type_Knot-", knot
  )
  stargazer(
    c(tmp_reg.result_a, tmp_reg.result_b, tmp_reg.result_c, tmp_reg.result_d),
    type = "text",
    title = title_,
    out = NULL,
    out.header = out.header,
  ...
  stargazer(
    ...
    out = paste(
      ...
      paste0(
        "Breakdown-of-ATEs_Hourly-in-the-Peak-Rate-Period_",
        "By-Tariff-and-Heating-Type_By-using-Spline-Regressions_",
        "Knot-", knot, ".tex"
      ),
      sep = "/"
    ),
...
# ------------------------------------------------------------------------------
# Create a transition matrix indicating how Irish households changes their
# heating types
# ------------------------------------------------------------------------------
# ## Note:
# ## By utilizing information obtained below, I create an excel file
# ## (i.e., "Transition-of-Heating-Type.xlsx")

# ------- Create object(s) that will be utilized later -------
cols_by_1 <- c(
  "id",
  "is_elec.heating_space_pre", "is_elec.heating_water_pre",
  "is_elec.heating_space_post", "is_elec.heating_water_post"
)
cols_by_2 <- cols_by_1[2:5]


# ------- Case 1:                                                      -------
# ------- (Elec. Heating for Space, Elec. Heating for Water) == (T, T) -------
# # 1. The number of households for the case in Pre-Survey
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    as.character(rate.period) == "Peak" &
    alloc_r_tariff %in% LETTERS[1:5] &
    is_elec.heating_space_pre == TRUE & is_elec.heating_water_pre == TRUE,
  .N,
  by = .(id)
][, .N]

# # 2. Stay- and out-related household numbers for the case
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    as.character(rate.period) == "Peak" &
    alloc_r_tariff %in% LETTERS[1:5] &
...
# # 3. In-related household numbers for the case
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    as.character(rate.period) == "Peak" &
    alloc_r_tariff %in% LETTERS[1:5] &
    (
      !(is_elec.heating_space_pre == TRUE & is_elec.heating_water_pre == TRUE) |
        (is.na(is_elec.heating_space_pre) | is.na(is_elec.heating_water_pre))
    ) &
    is_elec.heating_space_post == TRUE & is_elec.heating_water_post == TRUE,
  .N,
  by = cols_by_1
][
  , .N, keyby = cols_by_2
...
# # 4. The number of households for the case in Post-Survey
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    as.character(rate.period) == "Peak" &
    alloc_r_tariff %in% LETTERS[1:5] &
...
# ------- Case 2:                                                      -------
# ------- (Elec. Heating for Space, Elec. Heating for Water) == (T, F) -------
# # 1. The number of households for the case in Pre-Survey
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    as.character(rate.period) == "Peak" &
    alloc_r_tariff %in% LETTERS[1:5] &
    is_elec.heating_space_pre == TRUE & is_elec.heating_water_pre == FALSE,
  .N,
  by = .(id)
][
  , .N
...
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    ...
    is_elec.heating_space_post == TRUE & is_elec.heating_water_post == FALSE,
  .N,
  by = cols_by_1
][
  , .N, keyby = cols_by_2
...
# ------- Case 3:                                                      -------
# ------- (Elec. Heating for Space, Elec. Heating for Water) == (F, T) -------
# # 1. The number of households for the case in Pre-Survey
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    as.character(rate.period) == "Peak" &
    alloc_r_tariff %in% LETTERS[1:5] &
    is_elec.heating_space_pre == FALSE & is_elec.heating_water_pre == TRUE,
  .N,
  by = .(id)
][
  , .N
...
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    ...
    is_elec.heating_space_post == FALSE & is_elec.heating_water_post == TRUE,
  .N,
  by = cols_by_1
][
  , .N, keyby = cols_by_2
...
# ------- Case 4:                                                      -------
# ------- (Elec. Heating for Space, Elec. Heating for Water) == (F, F) -------
# # 1. The number of households for the case in Pre-Survey
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    as.character(rate.period) == "Peak" &
...
# ------- Case 5:                                                        -------
# ------- (Elec. Heating for Space, Elec. Heating for Water) == (NA, NA) -------
# # 1. The number of households for the case in Pre-Survey
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    as.character(rate.period) == "Peak" &
...
dt_for.reg[
  is_in.sample_incl.control_base.only.second.half == TRUE &
    ...
    alloc_r_tariff %in% LETTERS[1:5] &
    (!(is.na(is_elec.heating_space_pre) & is.na(is_elec.heating_water_pre))) &
    is.na(is_elec.heating_space_post) & is.na(is_elec.heating_water_post),
  .N,
  by = cols_by_1
][
  , .N, keyby = cols_by_2
