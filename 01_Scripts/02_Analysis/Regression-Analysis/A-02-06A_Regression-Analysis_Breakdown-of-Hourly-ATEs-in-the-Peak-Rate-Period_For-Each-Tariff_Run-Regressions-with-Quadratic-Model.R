...
# # : Run regressions, with a model incl. quadratic terms of HDDs, to breakdown
# #   hourly ATEs for each tariff. And then save the regression results in
# #   .RData format. This script needs about 2 hours to run all regressions.
...
DIR_TO.SAVE_REG.RESULTS <- paste(
  "/Volumes/Extreme-Pro/Energy-Demand-Analysis/Regression-Results/CER-Trial",
...
dt_for.reg[
  ...
  `:=` (
    ...
    treatment_by_hdd2 =
      as.numeric(is_treated_r) * (hdd_all_60f^2),
...
list_models <- list(
  model_breakdown.of.ate_hourly.in.peak_quadratic_dw.mw =
    model_breakdown.of.ate_hourly.in.peak_quadratic_dw.mw
)
  ...
  save(
    ...
    file = paste(
      ...
      paste0(
        ...
        "Hourly-in-the-Peak-Rate-Period_For-Each-Tariff_Quadratic-Model_",
...
save(
  ...
  file = paste(
    DIR_TO.SAVE_REG.RESULTS,
    paste0(
      "CER_Estimates_Breakdown-of-ATEs_Hourly-in-the-Peak-Rate-Period_",
      "For-Each-Tariff_Quadratic-Model.RData"
    ),
    sep = "/"
  )
)
