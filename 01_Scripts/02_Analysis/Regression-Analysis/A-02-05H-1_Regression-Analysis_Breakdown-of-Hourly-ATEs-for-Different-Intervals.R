...
# #   1) Treatment effect on electricity consumption for non-temperature-control
# #      uses and that for temperature-control uses, respectively
# #   2) For pre-peak, peak, and post-peak intervals
...
list_reg.results_breakdown.of.ates <- lapply(
  LIST_INTERVALS[1:3],
...
dt_estimates <- lapply(
  list_reg.results_breakdown.of.ates,
...
dt_for.plot <- merge(
  ...
  y = dt_estimates[
    ...
    setnames(., old = "estimate", "ate_non.temp"),
  by = "interval",
  all.x = TRUE
)
...
dt_for.plot[
  ,
  `:=` (
    predicted.kwh_non.temp = ate_non.temp,
    predicted.kwh_temp = (
      ate_temp_hdd * hdd +
...
plot_predicted.kwh_15to16 <-
  ggplot() +
    ...
    geom_line(
      data = dt_for.plot_melted[interval == "15 to 16"],
      aes(x = hdd, y = predicted.kwh, group = category),
      color = "black", alpha = 0.3, lwd = 1
    ) +
    geom_line(
      data = dt_for.plot_melted[interval == "15 to 16"],
      aes(x = hdd, y = predicted.kwh, color = category, group = category)
    ) +
    geom_point(
      data = dt_for.plot_melted[interval == "15 to 16"],
    ...
    geom_point(
      data = dt_for.plot_melted[interval == "15 to 16"],
      aes(x = hdd, y = predicted.kwh, color = category, shape = category),
      size = 2.0
    ) +
    scale_x_continuous(breaks = seq(0, 30, by = 5)) +
    scale_y_continuous(breaks = seq(-0.06, 0, by = 0.01)) +
    scale_color_manual(values = col.pal_custom[c(1, 2, 4)]) +
...
plot_predicted.kwh_17to18 <-
  ggplot() +
    ...
    geom_line(
      data = dt_for.plot_melted[interval == "17 to 18"],
      aes(x = hdd, y = predicted.kwh, group = category),
      color = "black", alpha = 0.3, lwd = 1
    ) +
    geom_line(
    ...
    scale_x_continuous(breaks = seq(0, 30, by = 5)) +
    scale_y_continuous(breaks = seq(-0.175, 0, by = 0.025)) +
    scale_color_manual(values = col.pal_custom[c(1, 2, 4)]) +
    scale_shape_manual(values = 15:17) +
    labs(
      x = "Heating Degree Days",
...
plot_predicted.kwh_19to20 <-
  ggplot() +
    ...
    geom_line(
      data = dt_for.plot_melted[interval == "19 to 20"],
      aes(x = hdd, y = predicted.kwh, group = category),
      color = "black", alpha = 0.3, lwd = 1
    ) +
    geom_line(
    ...
    scale_x_continuous(breaks = seq(0, 30, by = 5)) +
    scale_y_continuous(breaks = seq(-0.04, 0.06, by = 0.01)) +
    scale_color_manual(values = col.pal_custom[c(1, 2, 4)]) +
    scale_shape_manual(values = 15:17) +
    labs(
      x = "Heating Degree Days",
...
plot_predicted.kwh <-
  ggplot() +
    ...
    facet_wrap(
      interval ~ ., labeller = labeller(interval = labels_interval), nrow = 1
    ) +
    scale_x_continuous(breaks = seq(0, 30, by = 5)) +
    scale_color_manual(values = col.pal_custom[c(1, 2, 4)]) +
    scale_shape_manual(values = 15:17) +
...
export_figure.in.png(
  ...
  filename_str = paste(
    ...
    paste0(
      ...
      "For-Different-Intervals_Pre-Peak-Hours_Knot-",
      KNOT, ".png"
    ),
    sep = "/"
  ),
...
export_figure.in.png(
  ...
  filename_str = paste(
    ...
    paste0(
      "Figure_Breakdown-of-Hourly-ATEs_",
      "For-Different-Intervals_Peak-Hours_Knot-",
      KNOT, ".png"
    ),
    sep = "/"
  ),
...
export_figure.in.png(
  ...
  filename_str = paste(
    ...
    paste0(
      "Figure_Breakdown-of-Hourly-ATEs_",
      "For-Different-Intervals_Post-Peak-Hours_Knot-",
      KNOT, ".png"
    ),
    sep = "/"
  ),
...
export_figure.in.png(
  ...
  filename_str = paste(
    ...
    paste0(
      "Figure_Breakdown-of-Hourly-ATEs_",
      "For-Different-Intervals_All_Knot-",
      KNOT, ".png"
    ),
    sep = "/"
  ),
