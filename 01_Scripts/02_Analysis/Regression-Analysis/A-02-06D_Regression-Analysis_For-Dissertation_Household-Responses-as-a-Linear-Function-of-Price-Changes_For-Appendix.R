...
add.lines <- list(
  c(
    ...
    rep(c("Pre-Peak", "Peak", "Post-Peak"), times = 2)
  ),
  c("Period of Hours", rep(names(LIST_INTERVALS[1:3]), times = 2)),
  c("Knot", rep(KNOT, times = 6)),
  c("FEs: Household", rep(c("No", "Yes"), each = 3)),
...
stargazer(
  list_reg.results_dw, list_reg.results_i.dw,
  type = "text",
  title = title_,
