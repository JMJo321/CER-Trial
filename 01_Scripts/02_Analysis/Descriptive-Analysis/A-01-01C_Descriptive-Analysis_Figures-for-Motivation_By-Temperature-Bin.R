...
# #   This script is the same with A-01-01B except one thing that Panel B in
# #   this script shows percentage changes in electricity consumption.
...
help_compute.diff.in.kwh <- function (data.table_dt) {
  ...
  dt_to.return <- melt(
    dt_merged,
    id.vars =
      c("group", "range", "range_mid.point", "kwh", "percentage.change"),
  ...
  return(
    dt_to.return[
      ,
      .(group, period, range, kwh, percentage.change, range_mid.point, category)
    ]
  )
}
